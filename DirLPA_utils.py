...
def print_in_dist_values(py_in, targets, train='mnist', method='LLLA-KF'):
    acc_in = np.mean(np.argmax(py_in, 1) == targets)
    ents_in_laplace_kf = -np.sum(py_in*np.log(py_in+1e-8), axis=1)
    print(f'[In, {method}, {train}] Accuracy: {acc_in:.3f}; average entropy: {ents_in_laplace_kf.mean():.3f}; MMC: {py_in.max(1).mean():.3f}')


def print_out_dist_values(py_in, py_out, train='mnist', test='FMNIST', method='LLLA-KF'):
    ents_out_laplace_kf = -np.sum(py_out*np.log(py_out+1e-8), axis=1)
    labels = np.zeros(len(py_in)+len(py_out), dtype='int32')
    labels[:len(py_in)] = 1
    examples = np.concatenate([py_in.max(1), py_out.max(1)])
    auroc = roc_auc_score(labels, examples)
    print(f'[Out-{test}, {method}, {train}] Average entropy: {ents_out_laplace_kf.mean():.3f}; MMC: {py_out.max(1).mean():.3f}; AUROC: {auroc:.3f}')
